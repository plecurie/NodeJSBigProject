...
export class bulkindexService {
    ...
    public async importContracts(contractsxlsx= 'InputContracts.xlsx',
        ...
        for (let i = 0; i < Object.keys(contracts).length; i++) {
            ...
            contracts_list.push({index: {_index: index, _type: type}});
            contracts_list.push(this.mapToObj(dict_contracts));
        }
...
export class bulkindexService {
    ...
    public async importContracts(contractsxlsx= 'InputContracts.xslx',
                                 productsxlsx= 'InputBuyList.xslx'): Promise<boolean> {
        ...
        for (let i = 0; i < Object.keys(contracts).length; i++) {
            let buylist = [];
            for (let j = 0; j < Object.keys(products).length; j++) {
                if ( contracts[i] === products[j]['ID Contrat']) {
    ...
    public async importProducts(productsxlsx = 'InputProducts.xslx'): Promise<boolean> {
        const data = excelToJsonService.getInstance().processXlsxToJson(productsxlsx);
        const products_list = [];
        let previous = {};
